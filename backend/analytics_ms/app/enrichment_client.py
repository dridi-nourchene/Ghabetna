...
"""
Request-time enrichment for names / contact info.

Deliberately NOT a Redis-stream-fed cache: this data (agent/supervisor
name, phone, email; forest name) is only needed for admin-only,
low-QPS analytics endpoints, so a synchronous (but async/non-blocking)
call to forest_ms at request time avoids duplicating a second copy of
PII across three services. If this ever becomes a bottleneck, swap this
module's internals for a consumer of forest_ms's existing streams
without touching analytics_service.py's call sites.
"""
import httpx
